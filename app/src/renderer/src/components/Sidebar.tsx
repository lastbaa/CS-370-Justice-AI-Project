...
function FileCard({
  ...
  return (
    <div
      ...
      className="group relative rounded-lg border border-[#1e1e1e] bg-[#141414] px-3 py-2.5 hover:border-[#c9a84c]/40 transition-all"
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-start gap-2 min-w-0">
          ...
          <span className="text-sm font-medium text-[#ffffff] truncate" title={file.fileName}>
            {file.fileName}
          </span>
        </div>
        {hovered && (
    ...
    <div
      ...
      className={`group relative rounded-lg px-3 py-2.5 cursor-pointer transition-all ${
        isActive
          ...
          : 'border border-transparent hover:bg-[#141414] hover:border-[#1e1e1e]'
      }`}
    >
      <div className="flex items-start justify-between gap-2">
        <p
          className={`text-xs leading-snug truncate flex-1 ${isActive ? 'text-[#ffffff]' : 'text-[#8b949e]'}`}
          title={session.name}
        >
          {session.name}
        </p>
      ...
      <p className="mt-0.5 text-[10px] text-[#333333]">
...
export default function Sidebar({
  ...
  return (
    <aside className="flex h-screen w-56 flex-shrink-0 flex-col border-r border-[#1e1e1e] bg-[#0d0d0d]">
      {/* Header */}
      <div className="drag-region flex items-center justify-between px-4 py-3.5 border-b border-[#1e1e1e]">
        <div className="no-drag flex items-center gap-2">
          <ScalesLogo />
          <div>
            <h1 className="text-sm font-bold text-[#c9a84c] tracking-wide leading-none">Justice AI</h1>
            <p className="text-[10px] text-[#333333] leading-tight mt-0.5">Legal Research</p>
          </div>
        </div>
        {/* New chat button */}
        <button
          onClick={onNewChat}
          className="no-drag flex h-7 w-7 items-center justify-center rounded-lg border border-[#1e1e1e] text-[#8b949e] hover:text-[#ffffff] hover:border-[#8b949e] hover:bg-[#141414] transition-all"
          title="New chat"
        >
      ...
      <div className="flex border-b border-[#1e1e1e]">
        <button
          ...
          className={`flex-1 py-2 text-[11px] font-medium transition-colors ${
            activeTab === 'files'
              ? 'text-[#ffffff] border-b-2 border-[#c9a84c]'
              : 'text-[#8b949e] hover:text-[#ffffff]'
          }`}
        >
          Documents
          {files.length > 0 && (
            <span className="ml-1.5 rounded-full bg-[#1e1e1e] px-1.5 py-0.5 text-[10px] font-normal text-[#8b949e]">
              {files.length}
            </span>
          )}
        </button>
          ...
          <div className="px-3 py-3 border-b border-[#1e1e1e]">
            <div className="flex flex-col gap-1.5">
              <button
                ...
                disabled={isLoading}
                className="no-drag flex items-center gap-2 rounded-lg border border-[#1e1e1e] bg-transparent px-3 py-2 text-sm text-[#ffffff] hover:border-[#c9a84c]/60 hover:bg-[#c9a84c]/10 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
                {isLoading ? <Spinner /> : (
                ...
                <span>Add Files</span>
                <span className="ml-auto text-[10px] text-[#333333]">PDF, DOCX</span>
              </button>
              <button
            ...
            {files.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 text-center">
                <svg width="28" height="28" viewBox="0 0 16 16" fill="none" className="mb-2">
                  <path d="M2 1.75C2 .784 2.784 0 3.75 0h6.586c.464 0 .909.184 1.237.513l2.914 2.914c.329.328.513.773.513 1.237v9.586A1.75 1.75 0 0 1 13.25 16h-9.5A1.75 1.75 0 0 1 2 14.25V1.75z" fill="#141414" />
                </svg>
                <p className="text-xs text-[#8b949e]">No documents loaded</p>
                <p className="mt-0.5 text-[10px] text-[#333333]">Add files to begin</p>
              </div>
            ) : (
          ...
          {sessions.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 text-center">
              <svg width="24" height="24" viewBox="0 0 16 16" fill="none" className="mb-2">
                <path d="M1.5 8a6.5 6.5 0 1 1 13 0 6.5 6.5 0 0 1-13 0zM8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0zm.5 4.75a.75.75 0 0 0-1.5 0v3.5l-1.72 1.72a.75.75 0 1 0 1.06 1.06l2-2a.75.75 0 0 0 .22-.53V4.75z" fill="#1e1e1e" />
              </svg>
              <p className="text-xs text-[#8b949e]">No sessions yet</p>
              <p className="mt-0.5 text-[10px] text-[#333333]">Sessions save automatically</p>
            </div>
          ) : (
      ...
      <div className="border-t border-[#1e1e1e] px-3 py-3">
        <button
          ...
          className="no-drag flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-[#8b949e] hover:bg-[#141414] hover:text-[#ffffff] transition-all"
        >
          <svg width="13" height="13" viewBox="0 0 16 16" fill="currentColor">
