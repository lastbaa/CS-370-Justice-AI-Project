...
import { ChatSession, OllamaStatus } from '../../../../../shared/src/types'
    ...
    <div
      ...
      className={`group relative flex items-center gap-2 rounded-lg px-3 py-2 cursor-pointer transition-all ${
        isActive
          ...
          : 'text-[#585858] hover:bg-[#111] hover:text-[#999]'
      }`}
    >
      <span
      ...
      {hovered && (
        <button
          ...
          className="no-drag shrink-0 flex h-4 w-4 items-center justify-center rounded text-[#444] hover:text-[#f85149] transition-colors"
        >
...
export default function Sidebar({
  ...
  const statusDot = (() => {
    if (!ollamaStatus) return { color: '#3a3a3a', title: 'Ollama status unknown' }
    if (!ollamaStatus.running) return { color: '#f85149', title: 'Ollama not running' }
    if (!ollamaStatus.hasLlmModel || !ollamaStatus.hasEmbedModel) return { color: '#e3b341', title: 'Models not fully installed' }
    return { color: '#3fb950', title: 'Ready' }
  })()
    ...
    <aside className="flex h-screen w-[240px] shrink-0 flex-col border-r border-[#161616] bg-[#090909]">
      ...
      <div className="px-3 pb-2">
        <button
          onClick={onNewChat}
          title="New chat"
          className="no-drag flex h-6 w-6 items-center justify-center rounded-md text-[#444] hover:bg-[#1a1a1a] hover:text-[#aaa] transition-all"
        >
      ...
      <div className="flex-1 overflow-y-auto px-2">
        {sessions.length === 0 ? (
          <div className="flex flex-col items-center py-16 text-center px-4">
            ...
            <p className="text-[11px] text-[#2e2e2e]">No chats yet</p>
            <p className="mt-0.5 text-[10px] text-[#222]">Sessions auto-save</p>
          </div>
        ) : (
          <div className="flex flex-col gap-5 py-1">
            {groups.map((group) => (
              <div key={group.label}>
                <p className="mb-1 px-3 text-[10px] font-semibold uppercase tracking-[0.1em] text-[#2e2e2e]">
                  {group.label}
                </p>
      ...
      {/* ── Bottom actions ── */}
      <div className="border-t border-[#141414] px-2 py-2 flex flex-col gap-0.5">
        <button
          onClick={onAddFiles}
          disabled={isLoading}
          className="no-drag flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-[12px] text-[#484848] hover:bg-[#111] hover:text-[#999] disabled:opacity-40 transition-all"
        >
          <svg width="12" height="12" viewBox="0 0 16 16" fill="#c9a84c">
        ...
        <button
          ...
          className="no-drag flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-[12px] text-[#484848] hover:bg-[#111] hover:text-[#999] transition-all"
