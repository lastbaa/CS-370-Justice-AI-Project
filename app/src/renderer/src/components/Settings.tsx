...
function Field({
  ...
  return (
    <div>
      <label className="block text-sm font-medium text-[#e6edf3] mb-1">{label}</label>
...
function TextInput({
  ...
  return (
    <input
      ...
      className="w-full rounded-lg border border-[#30363d] bg-[#0d1117] px-3 py-2 text-sm text-[#e6edf3] placeholder-[#8b949e] outline-none focus:border-[#c9a84c]/60 transition-colors"
    />
  )
}
...
function NumberInput({
  ...
  return (
    <input
      ...
      min={min}
      max={max}
      className="w-full rounded-lg border border-[#30363d] bg-[#0d1117] px-3 py-2 text-sm text-[#e6edf3] outline-none focus:border-[#c9a84c]/60 transition-colors"
    />
  )
}
...
function SectionHeader({ children }: { children: React.ReactNode }): JSX.Element {
  return (
    <h3 className="text-xs font-semibold uppercase tracking-widest text-[#8b949e] mb-4 pb-2 border-b border-[#30363d]">
      {children}
    </h3>
  )
}
...
export default function Settings({ settings, onSave, onClose }: Props): JSX.Element {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-xl border border-[#30363d] bg-[#161b22] shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-[#30363d] px-6 py-4">
          ...
          <button
            onClick={onClose}
            className="flex h-7 w-7 items-center justify-center rounded-lg text-[#8b949e] hover:bg-[#21262d] hover:text-[#e6edf3] transition-colors"
          >
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-3 border-t border-[#30363d] px-6 py-4">
          <button
            onClick={onClose}
            className="rounded-lg border border-[#30363d] bg-transparent px-4 py-2 text-sm text-[#8b949e] hover:text-[#e6edf3] hover:border-[#8b949e] transition-colors"
          >
            Cancel
          </button>
          <button
            onClick={handleSave}
            className="rounded-lg bg-[#c9a84c] px-4 py-2 text-sm font-semibold text-[#0d1117] hover:bg-[#e8c97e] transition-colors"
          >
            Save Settings
          </button>
        </div>
